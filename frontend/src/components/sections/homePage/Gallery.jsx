...
const Gallery = () => {
    ...
    // Split into pages of 6 images each (dynamic slide count)
    const pages = useMemo(() => {
        const chunkSize = 6;
        const chunks = [];
    ...
    const lightboxNext = () =>
        setLightboxIndex((i) =>
            i === null ? null : (i + 1) % flatImages.length
        );

    const currentImages = pages[pageIndex] || [];

    // Auto-advance slides every 6 seconds (loop through dynamically)
    useEffect(() => {
    ...
    return (
        <section
            id="gallery"
            className="relative bg-gradient-to-br from-gray-50 via-white to-gray-100 py-12"
        >
            {/* Background decoration */}
            <div
            ...
            <div className="relative mx-auto max-w-7xl px-4">
                ...
                <div className="text-center">
                    ...
                    <p className="mt-4 max-w-3xl mx-auto text-lg text-gray-600 leading-relaxed">
                        Explore our portfolio of premium fit-outs and
                        renovations. Each project reflects our commitment to
                        excellence and attention to detail.
                    </p>
                </div>
                ...
                {/* Dynamic gallery */}
                <div
                    className={`mt-12 grid gap-0 sm:grid-cols-2 lg:grid-cols-3 transition-all duration-500 ${isFading
                        ? "opacity-0 scale-95"
                ...
                {lightboxIndex !== null && (
                    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/90 p-4">
                        <button
                            ...
                            className="absolute right-6 top-6 rounded-md bg-white/10 px-3 py-1 text-white backdrop-blur hover:bg-white/20"
                        >
                            Close
                        </button>
                        <button
